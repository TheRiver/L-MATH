...
(defgeneric c+ (lhs rhs)
  ...
  (:method ((lhs number) (rhs matrix))
    (let ((result (make-matrix (matrix-rows rhs)
			       (matrix-cols rhs))))
      (do-each-matrix-element (item rhs row col)
	(setf (matrix-elt result row col) (+ item lhs)))
      result))
  (:method ((lhs matrix) (rhs number))
    (c+ rhs lhs)))
...
(defgeneric c- (lhs rhs)
  ...
  (:method ((lhs number) (rhs matrix))
    (let ((result (make-matrix (matrix-rows rhs)
			       (matrix-cols rhs))))
      (do-each-matrix-element (item rhs row col)
	(setf (matrix-elt result row col) (- lhs item)))
      result))
  (:method ((lhs matrix) (rhs number))
    (c- (- rhs) lhs)))
...
(defgeneric c/ (lhs rhs)
  ...
  (:method ((lhs number) (rhs matrix))
    (let ((result (make-matrix (matrix-rows rhs)
			       (matrix-cols rhs))))
      (do-each-matrix-element (item rhs row col)
	(setf (matrix-elt result row col) (cl:/ lhs item)))
      result))
  (:method ((lhs matrix) (rhs number))
    (let ((result (make-matrix (matrix-rows lhs)
			       (matrix-cols lhs))))
      (do-each-matrix-element (item lhs row col)
	(setf (matrix-elt result row col) (cl:/ item rhs)))
      result)))
